#!/usr/bin/env node
/**
 * Honest cold bench: ood_fresh6 + hard LongBench mix, seed 3377. No mid-run engine tuning..
...
const SEED = Number(process.env.SC_SEED || 3377);
...
// Harder never-used mix at seed 3377 (musique multi-hop + trivia dumps + narrative + 2wiki).
const NEW_LONGBENCH = [
...
async function main() {
  ...
  const hardPath = path.join(HELDOUT, "ood_fresh6", "hard_cases.jsonl");
  if (!fs.existsSync(hardPath)) {
    console.error("missing hard dataset — run: node scripts/build_ood_fresh6.js");
    process.exit(1);
  }
  const hardLines = fs
  ...
  const outPath = path.join(WEB, "data", "fresh6-benchmark-latest.json");
