#!/usr/bin/env node
/**
 * Real anti-overfit bench on a brand-new dataset:
 *   1) ood_fresh3_hard — never-before-used wiki/RFC/READMEs buried in distractors,
 *      single rare gold answers (not title keywords).
 *   2) Never-used LongBench tasks: hotpotqa / 2wikimqa / multifieldqa_en / qasper
 *      shuffled with SEED 4242 (never used in tuning or prior validate).
 *
 * Primary metric: gold answer containment in compressed text.
 * Does NOT overwrite validate/fresh marketing artifacts.
 */
const fs = require("fs");
...
const SEED = Number(process.env.SC_SEED || 4242);
...
function runOne(context, query, answers, meta) {
  ...
  // Only score answers that literally appear in the (clipped) context.
  // Skip yes/no-only golds — string containment of "Yes" is not retention signal.
  ...
  const present = list.filter((a) => {
    ...
    return norm(ctx).includes(needle);
  });
...
async function main() {
  ...
  // --- brand-new hard OOD haystacks ---
  const hardPath = path.join(HELDOUT, "ood_fresh3", "hard_cases.jsonl");
  if (!fs.existsSync(hardPath)) {
    console.error("missing hard dataset:", hardPath);
    process.exit(1);
  }
  const hardLines = fs
  ...
  const out = {
    meta: {
      real_bench: true,
      note:
        "Brand-new dataset: ood_fresh3_hard (rare gold buried in distractors) + never-used LongBench tasks (hotpotqa/2wikimqa/multifieldqa/qasper). Seed 4242. Primary = answer containment. Not a marketing artifact.",
      seed: SEED,
      per_task: PER_TASK,
      tuned_against_seeds: [777, 2026],
  ...
  out.gates.pass = Boolean(out.gates.avg_cut_ge_55 && out.gates.always_answer_ge_98);

  const outPath = path.join(WEB, "data", "real-benchmark-latest.json");
