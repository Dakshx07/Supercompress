#!/usr/bin/env node
/**
 * Brand-new anti-overfit bench (ood_fresh4 + never-used LongBench mix + seed 9091).
...
const SEED = Number(process.env.SC_SEED || 9091);
...
// Never used in seed-4242 real bench task mix
const NEW_LONGBENCH = [
...
async function main() {
  ...
  const hardPath = path.join(HELDOUT, "ood_fresh4", "hard_cases.jsonl");
  if (!fs.existsSync(hardPath)) {
    console.error("missing hard dataset — run: node scripts/build_ood_fresh4.js");
    process.exit(1);
  }
  const hardLines = fs
  ...
  for (const s of hardLines) {
    const row = await runOne(s.context || "", s.input || "", s.answers || [], {
      task: "ood_fresh4_hard",
      id: s.id || null,
    });
    hardRows.push(row);
    all.push(row);
    process.stderr.write(
      `.ood_fresh4_hard ${hardRows.length}/${hardLines.length} cut=${row.tokens_saved_pct}% ans=${row.answer_all_kept} id=${row.id}\n`
    );
  }
  byTask.ood_fresh4_hard = summarize(hardRows);
  ...
  const out = {
    meta: {
      fresh4_bench: true,
      note:
        "Brand-new dataset: ood_fresh4_hard (new wiki/RFC/READMEs buried in distractors) + never-used LongBench mix (musique/triviaqa/narrativeqa/samsum). Seed 9091. Primary = answer containment.",
      seed: SEED,
      per_task: PER_TASK,
      prior_real_bench_seed: 4242,
      tuned_against_seeds: [777, 2026, 4242],
  ...
  const outPath = path.join(WEB, "data", "fresh4-benchmark-latest.json");
